...
from Fsec import YouTube, app
from Fsec.core.call import Fsec
from Fsec.misc import SUDOERS, db
from Fsec.utils import bot_sys_stats
from Fsec.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from Fsec.utils.decorators.language import languageCB
from Fsec.utils.formatters import seconds_to_min
from Fsec.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from Fsec.utils.stream.autoclear import auto_clean
from Fsec.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Fsec.pause_stream(chat_id)
        ...
        await Fsec.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await Fsec.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Fsec.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Fsec.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Fsec.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
